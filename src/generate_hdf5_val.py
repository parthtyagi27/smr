...
# Generate a batch of waveforms
def generate_batch_of_waveforms(batch):
    with multiprocessing.Pool() as pool:
        return pool.map(generate_waveform, batch)
...
if __name__ == "__main__":
    ...
    li = generate_indices(step_q=8*(np.pi/150), step_chi=np.pi/100)
    chunk = 32  # Number of waveforms to generate on each rank at each iter [Limited by memory on the node]

    # Create hdf5 file on rank 0
    if rank == 0:
        f = h5py.File("val.hdf5", "w")
