...
if __name__ == "__main__":
    ...
    q_val = [(q_train[i] + q_train[i+1])/2 for i in range(1, len(q_train)-1, 2)]
    s1_val = [(s1_train[i] + s1_train[i+1])/2 for i in range(1, len(s1_train)-1, 2)]
    s2_val = [(s2_train[i] + s2_train[i+1])/2 for i in range(1, len(s2_train)-1, 2)]

    new_val_li = []
    for q in q_val:
        for chiA in s1_val:
            for chiB in s2_val:
                new_val_li.append((q,chiA,chiB))
    new_val_li = np.asarray(new_val_li)
    ## ##
